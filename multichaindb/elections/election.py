...
from multichaindb.common.exceptions import (InvalidSignature,
                                          MultipleInputsError,
                                          InvalidProposer,
                                          UnequalValidatorSet,
                                          DuplicateTransaction)
from multichaindb.tendermint_utils import key_from_base64, public_key_to_base64
from multichaindb.common.crypto import (public_key_from_ed25519_key)
from multichaindb.common.transaction import Transaction
from multichaindb.common.schema import (_validate_schema,
                                      TX_SCHEMA_COMMON,
                                      TX_SCHEMA_CREATE)
...
class Election(Transaction):
    ...
    @classmethod
    def get_validator_change(cls, multichain):
        ...
        """
        latest_block = multichain.get_latest_block()
        if latest_block is None:
            return None
        return multichain.get_validator_change(latest_block['height'])


    @classmethod
    def get_validators(cls, multichain, height=None):
        """Return a dictionary of validators with key as `public_key` and
           value as the `voting_power`
        """
        validators = {}
        for validator in multichain.get_validators(height):
    ...
    @classmethod
    def recipients(cls, multichain):
        """Convert validator dictionary to a recipient list for `Transaction`"""

        recipients = []
        for public_key, voting_power in cls.get_validators(multichain).items():
    ...
    def validate(self, multichain, current_transactions=[]):
        ...
        * A valid election is one where voters are validators and votes are
        ...
        Args:
            :param multichain: (MultiChainDB) an instantiated multichaindb.lib.MultiChainDB object.
        ...
        if multichain.is_committed(self.id) or duplicates:
        ...
        current_validators = self.get_validators(multichain)
    ...
    def get_commited_votes(self, multichain, election_pk=None):
        if election_pk is None:
            election_pk = self.to_public_key(self.id)
        txns = list(backend.query.get_asset_tokens_for_public_key(multichain.connection,
                                                                  self.id,
                                                                  election_pk))
        return self.count_votes(election_pk, txns, dict.get)


    def has_concluded(self, multichain, current_votes=[]):
        ...
        * Elections can be concluded only if the current votes form a supermajority.
        ...
        """
        if self.has_validator_set_changed(multichain):
            return False

        election_pk = self.to_public_key(self.id)
        votes_committed = self.get_commited_votes(multichain, election_pk)
    ...
    def get_status(self, multichain):
        election = self.get_election(self.id, multichain)
        if election and election['is_concluded']:
            return self.CONCLUDED

        return self.INCONCLUSIVE if self.has_validator_set_changed(multichain) else self.ONGOING


    def has_validator_set_changed(self, multichain):
        latest_change = self.get_validator_change(multichain)
    ...
    def get_election(self, election_id, multichain):
        return multichain.get_election(election_id)


    def store(self, multichain, height, is_concluded):
        multichain.store_election(self.id, height, is_concluded)
    ...
    def show_election(self, multichain):
        ...
        for k, v in data.items():
            if k != 'seed':
                response += f'{k}={v}\n'
        response += f'status={self.get_status(multichain)}'
    ...
    @classmethod
    def process_block(cls, multichain, new_height, txns):
        ...
        if initiated_elections:
            multichain.store_elections(initiated_elections)
        ...
        for election_id, votes in elections.items():
            election = multichain.get_transaction(election_id)
            if election is None:
                continue

            if not election.has_concluded(multichain, votes):
                continue

            validator_update = election.on_approval(multichain, new_height)
            election.store(multichain, new_height, is_concluded=True)

        return [validator_update] if validator_update else []


    @classmethod
    def rollback(cls, multichain, new_height, txn_ids):
        ...
        multichain.delete_elections(new_height)

        txns = [multichain.get_transaction(tx_id) for tx_id in txn_ids]

        elections = cls._get_votes(txns)
        for election_id in elections:
            election = multichain.get_transaction(election_id)
            election.on_rollback(multichain, new_height)


    def on_approval(self, multichain, new_height):
