...
from multichaindb.common.exceptions import InvalidPowerChange
from multichaindb.elections.election import Election
from multichaindb.common.schema import TX_SCHEMA_VALIDATOR_ELECTION
...
class ValidatorElection(Election):
    ...
    def validate(self, multichaindb, current_transactions=[]):
        """For more details refer BEP-21: https://github.com/bigchaindb/BEPs/tree/master/21
        """

        current_validators = self.get_validators(multichaindb)

        super(ValidatorElection, self).validate(multichaindb, current_transactions=current_transactions)
    ...
    def has_concluded(self, multichaindb, *args, **kwargs):
        latest_block = multichaindb.get_latest_block()
        if latest_block is not None:
            latest_block_height = latest_block['height']
            latest_validator_change = multichaindb.get_validator_change()['height']
    ...
    def on_approval(self, multichaindb, new_height):
        validator_updates = [self.asset['data']]
        curr_validator_set = multichaindb.get_validators(new_height)
        ...
        multichaindb.store_validator_set(new_height + 1, updated_validator_set)
        return encode_validator(self.asset['data'])


    def on_rollback(self, multichaindb, new_height):
        # TODO change to `new_height + 2` when upgrading to Tendermint 0.24.0.
        multichaindb.delete_validator_set(new_height + 1)
