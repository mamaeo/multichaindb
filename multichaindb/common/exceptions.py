...
"""
from multichaindb.exceptions import MultiChainDBError


class ConfigurationError(MultiChainDBError):
    """Raised when there is a problem with server configuration"""


class DatabaseDoesNotExist(MultiChainDBError):
    """Raised when trying to delete the database but the db is not there"""


class StartupError(MultiChainDBError):
    """Raised when there is an error starting up the system"""


class CyclicBlockchainError(MultiChainDBError):
    """Raised when there is a cycle in the blockchain"""


class KeypairMismatchException(MultiChainDBError):
    """Raised if the private key(s) provided for signing don't match any of the
    current owner(s)
    """


class OperationError(MultiChainDBError):
...
class ValidationError(MultiChainDBError):
