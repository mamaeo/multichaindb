...
from multichaindb.core import rollback
from multichaindb.migrations.chain_migration_election import ChainMigrationElection
from multichaindb.utils import load_node_key
from multichaindb.common.transaction_mode_types import BROADCAST_TX_COMMIT
from multichaindb.common.exceptions import (DatabaseDoesNotExist,
                                          ValidationError)
from multichaindb.elections.vote import Vote
import multichaindb
from multichaindb import (backend, ValidatorElection,
                        MultiChainDB)
from multichaindb.backend import schema
from multichaindb.commands import utils
from multichaindb.commands.utils import (configure_multichaindb,
                                       input_on_stderr)
from multichaindb.log import setup_logging
from multichaindb.tendermint_utils import public_key_from_base64
from multichaindb.commands.election_types import elections
...
def run_configure(args):
    ...
    config_path = args.config or multichaindb.config_utils.CONFIG_DEFAULT_PATH
    ...
    conf = copy.deepcopy(multichaindb.config)
    ...
    database_keys = multichaindb._database_keys_map[args.backend]
    conf['database'] = multichaindb._database_map[args.backend]
    ...
    if config_path != '-':
        multichaindb.config_utils.write_config(conf, config_path)
    else:
...
def run_election_new(args, multichaindb):
    election_type = args.election_type.replace('-', '_')
    globals()[f'run_election_new_{election_type}'](args, multichaindb)


def create_new_election(sk, multichaindb, election_class, data):
    try:
        key = load_node_key(sk)
        voters = election_class.recipients(multichaindb)
        election = election_class.generate([key.public_key],
                                           voters,
                                           data, None).sign([key.private_key])
        election.validate(multichaindb)
    ...
    resp = multichaindb.write_transaction(election, BROADCAST_TX_COMMIT)
    if resp == (202, ''):
...
def run_election_new_upsert_validator(args, multichain):
    """Initiates an election to add/update/remove a validator to an existing MultiChainDB network
    ...
    return create_new_election(args.sk, multichain, ValidatorElection, new_validator)


def run_election_new_chain_migration(args, multichain):
    ...
    return create_new_election(args.sk, multichain, ChainMigrationElection, {})


def run_election_approve(args, multichain):
    """Approve an election
    ...
    tx = multichain.get_transaction(args.election_id)
    ...
    approval.validate(multichain)

    resp = multichain.write_transaction(approval, BROADCAST_TX_COMMIT)
...
def run_election_show(args, multichain):
    ...
    election = multichain.get_transaction(args.election_id)
    if not election:
        logger.error(f'No election found with election_id {args.election_id}')
        return

    response = election.show_election(multichain)
...
def run_drop(args):
    """Drop the database"""
    dbname = multichaindb.config['database']['name']
...
def run_start(args):
    ...
    logger.info('MultiChainDB Version %s', multichaindb.__version__)
    run_recover(multichaindb.lib.MultiChainDB())

    if not args.skip_initialize_database:
        logger.info('Initializing database')
        _run_init()

    logger.info('Starting MultiChainDB main process.')
    from multichaindb.start import start
    start(args)
...
def run_tendermint_version(args):
    ...
    supported_tm_ver = {
        'description': 'MultiChainDB supports the following Tendermint version(s)',
...
def create_parser():
    parser = argparse.ArgumentParser(
        description='Control your MultiChainDB node.',
    ...
    config_parser.add_argument('backend',
                               choices=['localarangodb'],
                               default='localarangodb',
                               const='localarangodb',
                               nargs='?',
                               help='The backend to use. It can only be '
                               '"localarangodb", currently.')
