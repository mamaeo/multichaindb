...
The value of each MultiChainDB Server configuration setting is
...
from multichaindb.common import exceptions
...
CONFIG_DEFAULT_PATH = os.environ.setdefault(
    'MULTICHAINDB_CONFIG_PATH',
    os.path.join(os.path.expanduser('~'), '.multichaindb'),
)

CONFIG_PREFIX = 'MULTICHAINDB'
CONFIG_SEP = '_'
...
def set_config(config):
    """Set multichaindb.config equal to the default config dict,
    then update that with whatever is in the provided config dict,
    and then set multichaindb.config['CONFIGURED'] = True
    ...
    Note:
        Any previous changes made to ``multichaindb.config`` will be lost.
    """
    # Deep copy the default config into bigchaindb.config
    multichaindb.config = copy.deepcopy(multichaindb._config)
    # Update the default config with whatever is in the passed config
    update(multichaindb.config, update_types(config, multichaindb.config))
    multichaindb.config['CONFIGURED'] = True
...
def is_configured():
    return bool(multichaindb.config.get('CONFIGURED'))
