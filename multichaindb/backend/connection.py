...
import multichaindb
from multichaindb.backend.exceptions import ConnectionError
from multichaindb.backend.utils import get_multichaindb_config_value, get_multichaindb_config_value_or_key_error
from multichaindb.common.exceptions import ConfigurationError

BACKENDS = {
    'localarangodb': 'multichaindb.backend.localarangodb.connection.LocalArangoDBConnection',
}
...
def connect(backend=None, host=None, port=None, name=None, max_tries=None,
    ...
    backend = backend or get_multichaindb_config_value_or_key_error('backend')
    host = host or get_multichaindb_config_value_or_key_error('host')
    port = port or get_multichaindb_config_value_or_key_error('port')
    dbname = name or get_multichaindb_config_value_or_key_error('name')
    ...
    # if it ever happens.
    replicaset = replicaset or get_multichaindb_config_value('replicaset')
    ssl = ssl if ssl is not None else get_multichaindb_config_value('ssl', False)
    login = login or get_multichaindb_config_value('login')
    password = password or get_multichaindb_config_value('password')
    ca_cert = ca_cert or get_multichaindb_config_value('ca_cert')
    certfile = certfile or get_multichaindb_config_value('certfile')
    keyfile = keyfile or get_multichaindb_config_value('keyfile')
    keyfile_passphrase = keyfile_passphrase or get_multichaindb_config_value('keyfile_passphrase', None)
    crlfile = crlfile or get_multichaindb_config_value('crlfile')
    ...
    try:
        ...
        raise ConfigurationError('Backend `{}` is not supported. '
                                 'MultiChainDB currently supports {}'.format(backend, BACKENDS.keys()))
...
class Connection:
    ...
    def __init__(self, host=None, port=None, dbname=None,
        ...
        dbconf = multichaindb.config['database']
