...
import multichaindb
from multichaindb import backend, config_utils, fastquery
from multichaindb.models import Transaction
from multichaindb.common.exceptions import (SchemaValidationError,
                                          ValidationError,
                                          DoubleSpend)
from multichaindb.common.transaction_mode_types import (BROADCAST_TX_COMMIT,
                                                      BROADCAST_TX_ASYNC,
                                                      BROADCAST_TX_SYNC)
from multichaindb.tendermint_utils import encode_transaction, merkleroot
from multichaindb import exceptions as core_exceptions
from multichaindb.validation import BaseValidationRules
...
class MultiChainDB(object):
    ...
    def __init__(self, connection=None):
        """Initialize the MultiChainDB instance

        A MultiChainDB instance has several configuration parameters (e.g. host).
        If a parameter value is passed as an argument to the MultiChainDB
        ...
        its default value (defined in multichaindb.__init__).

        Args:
            connection (:class:`~multichaindb.backend.connection.Connection`):
                A connection to the database.
        """
        # Questo metodo è incluso nello script config_utils.py
        config_utils.autoconfigure()
        self.mode_commit = BROADCAST_TX_COMMIT
        self.mode_list = (BROADCAST_TX_ASYNC,
                          BROADCAST_TX_SYNC,
                          self.mode_commit)
        self.tendermint_host = multichaindb.config['tendermint']['host']
        self.tendermint_port = multichaindb.config['tendermint']['port']
        self.endpoint = 'http://{}:{}/'.format(self.tendermint_host, self.tendermint_port)

        validationPlugin = multichaindb.config.get('validation_plugin')
        
        # Aggiunge nuove regole di validazione delle transazioni (se presenti) o assume le regole standard
        if validationPlugin:
            self.validation = config_utils.load_validation_plugin(validationPlugin)
        else:
            self.validation = BaseValidationRules
        
        # Crea una connessione all'istanza del database ArangoDB locale
        self.connection = connection if connection else backend.connect(**multichaindb.config['database'])
    ...
    def update_utxoset(self, transaction):
        ...
        Args:
            transaction (:obj:`~multichaindb.models.Transaction`): A new
